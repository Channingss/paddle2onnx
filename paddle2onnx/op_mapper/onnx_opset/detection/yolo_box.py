...
class YOLOBox():
    ...
    self.outputs_pred_box_x1_decode = ['yolo_box' + "@pred_box_x1_decode"]
    self.outputs_pred_box_y1_decode = ['yolo_box' + "@pred_box_y1_decode"]
    self.outputs_pred_box_x2_decode = ['yolo_box' + "@pred_box_x2_decode"]
    self.outputs_pred_box_y2_decode = ['yolo_box' + "@pred_box_y2_decode"]

    def front(cls, node, **kw):
        model_name = node.output('Boxes', 0)
        input_shape = node.output_shape('X', 0)
        ...
        node_x_reshape = onnx.helper.make_node(
            'Reshape',
            inputs=node.input('X') + name_x_shape,
            outputs=outputs_x_reshape)
        ...
        outputs_anchors_div_input_size = [
            model_name + "@anchors_div_input_size"
        ]
        ...
        node_img_width_cast = onnx.helper.make_node(
            'Cast',
            inputs=output_img_width,
            outputs=output_img_width_cast,
            to=1)
        ...
        node_img_height_cast = onnx.helper.make_node(
            'Cast',
            inputs=output_img_height,
            outputs=output_img_height_cast,
            to=1)
        ...
        node_pred_box_x1_decode = onnx.helper.make_node(
            ...
            outputs=self.outputs_pred_box_x1_decode)
        ...
        node_pred_box_y1_decode = onnx.helper.make_node(
            ...
            outputs=self.outputs_pred_box_y1_decode)
        ...
        node_pred_box_x2_decode = onnx.helper.make_node(
            ...
            outputs=self.outputs_pred_box_x2_decode)
        ...
        node_pred_box_y2_decode = onnx.helper.make_node(
            ...
            outputs=self.outputs_pred_box_y2_decode)
        ...
        node_pred_box_x2_sub_w = onnx.helper.make_node(
            'Sub',
            inputs=self.outputs_pred_box_x2_decode + output_new_img_width,
    ...
    def opset_9(cls, node, **kw):
        ...
        node_pred_box_x1_clip = onnx.helper.make_node(
            'Clip',
            inputs=self.outputs_pred_box_x1_decode,
        ...
        node_pred_box_y1_clip = onnx.helper.make_node(
            'Clip',
            inputs=self.outputs_pred_box_y1_decode,
        ...
        node_pred_box_x2_res = onnx.helper.make_node(
            'Sub',
            inputs=self.outputs_pred_box_x2_decode + outputs_pred_box_x2_clip,
        ...
        node_pred_box_y2_res = onnx.helper.make_node(
            'Sub',
            inputs=self.outputs_pred_box_y2_decode + outputs_pred_box_y2_clip,
    ...
    def opset_10(cls, node, **kw):
        ...
        node_pred_box_result = onnx.helper.make_node(
            'Concat',
            inputs=self.outputs_pred_box_x1_clip + outputs_pred_box_y1_clip +
            outputs_pred_box_x2_res + outputs_pred_box_y2_res,
