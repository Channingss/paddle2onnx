...
class BoxCoder():
    ...
    def opset_9(cls, node, **kw):
        ...
        normalized = node.attrs('normalized')
        if normalized == False:
            logging.warn(
        ...
        name_score_threshold = [result_name + "@score_threshold"]
        name_iou_threshold = [result_name + "@iou_threshold"]
        name_keep_top_k = [result_name + '@keep_top_k']
        name_keep_top_k_2D = [result_name + '@keep_top_k_1D']
        ...
        node_score_threshold = onnx.helper.make_node(
            ...
            value=onnx.helper.make_tensor(
                ...
                dims=(),
                vals=[float(node.attrs('score_threshold'))]))
        ...
        node_iou_threshold = onnx.helper.make_node(
            ...
            value=onnx.helper.make_tensor(
                ...
                data_type=onnx.TensorProto.FLOAT,
                dims=(),
                vals=[float(node.attr('nms_threshold'))]))
        ...
        node_keep_top_k = onnx.helper.make_node(
            ...
            value=onnx.helper.make_tensor(
                ...
                dims=(),
                vals=[np.int64(node.attrs('keep_top_k'))]))
        ...
        name_select_nms = [result_name + "@select_index"]
        node_select_nms= onnx.helper.make_node(
            'NonMaxSuppression',
            inputs=node.input('BBoxes') + node.input('Scores') + name_keep_top_k +\
        ...
        node_reshape_scores_rank1 = onnx.helper.make_node(
            "Reshape",
            inputs=node.input('Scores') + [result_name + "@const_-1"],
        ...
        node_shape_scores = onnx.helper.make_node(
            'Shape', inputs=node.input('Scores'), outputs=outputs_shape_scores)
        ...
        outputs_concat_topK_select_num = [
            result_name + "@conat_topK_select_num"
        ]
        ...
        node_gather_select_boxes = onnx.helper.make_node(
            'Gather',
            inputs=node.input('BBoxes') + outputs_squeeze_topk_boxes_id,
        ...
        node_topk_select_topk_indices = onnx.helper.make_node(
            'TopK',
            inputs=outputs_neg_squeeze_cast_topk_class +
            outputs_cast_topk_indices,
            outputs=outputs_topk_select_classes_indices)
        node_list.append(node_topk_select_topk_indices)
        outputs_concat_final_results = node.output('Out')
